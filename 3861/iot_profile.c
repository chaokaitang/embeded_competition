...
#define CN_PROFILE_TOPICFMT_CMDRESP "$oc/devices/%s/sys/commands/response/request_id=%s"
int IoTProfileCmdResp(char *deviceID, IoTCmdResp_t *payload)
{
    int ret = -1;
    const char *topic;
    const char *msg;
...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    // cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionFalldown,
    //                         payload->reportAction.FalldownActionData);
    // cJSON_AddNumberToObject(reported, payload->reportAction.Sensor_node,
    // payload->reportAction.Sensor_node_data);
    // cJSON_AddNumberToObject(reported, payload->reportAction.Control_node,
    // payload->reportAction.Control_node_data);
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
...
#define CN_PROFILE_TOPICFMT_TOPIC "$oc/devices/664f6c3cc2c0cb155cbe0cd3_Hispark/sys/properties/report"
