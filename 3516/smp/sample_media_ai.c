...
char uartReadBuff[32] ; /* 读取buff大小为2048 */
    unsigned int ret = 0;
   int  RecvLen=1024;
...
static HI_VOID PlayAudio(const int num, int time){
    audio_args[0] = num;
    audio_args[1] = time;
    pthread_create(&audio_thread, NULL, Audio_thread, (void*)audio_args);  // 播放对应语音
    // }
