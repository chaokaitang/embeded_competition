...
#define MODEL_FILE_GESTURE    "/userdata/models/hand_classify/face_classify_wk_inst.wk" // darknet framework wk model
#include <opencv2/core.hpp>
#include <opencv2/highgui.hpp>
#include <opencv2/imgproc.hpp>
using namespace std;
...
//     }
    ...
    if (biggestBoxIndex >= 0) {
        ...
        if (client_socket == -1) {
            ...
            packet.resize(sizeof(objBoxs[0]) + encoded_image.size());
            

            if (biggestBoxIndex >= 0) {
            objBoxs[0] = boxs[biggestBoxIndex];
            // 将 ObjectBox 结构体添加到数据包前面
            memcpy(packet.data(), &objBoxs[0], sizeof(objBoxs[0]));
            }
            else{
                memcpy(packet.data(), &emptyBox, sizeof(emptyBox));
            }

            
            // 将编码后的图像数据添加到数据包后面
            memcpy(packet.data() + sizeof(objBoxs[0]), encoded_image.data(), encoded_image.size());

        
            if (send(client_socket, packet.data(), packet.size(), 0) == -1) {
                std::cout << "Send failed" << std::endl;
                close(client_socket);   
            }
                else{
